var initSelect = function (context) {
    ...
    var preview = $('#preview', context).load(function () {
        ...
        //var button2 = $('button', context).attr('style', "visibility:visible;");
    });
    //alert(preview);
    ...
    var setPreview = function (x, y, w, h) {
        ...
        var LogoX = $('.LogoX');
        //debugger;
        $('.LogoX', context).val(x || 0);
        $('.LogoY', context).val(y || 0);
        $('.LogoWidth', context).val(w || preview[0].naturalWidth);
        $('.LogoHeight', context).val(h || preview[0].naturalHeight);
    };
    ...
    $('#Url', context).change(function () {
        ...
        //alert("Changed");
        setBox('#IsUrl');
        $('#uploadLogoAJAXButton').attr('disabled', false);
        $('#uploadLogoAJAXButton').css("visibility", "visible");
        $('#uploadLogoFULLButton').attr('disabled', true);
        $('#uploadLogoFULLButton').css("visibility", "hidden");
        preview.attr('src', this.value);
        URLSource = this.value;
    });

    //What happens if the File changes?
    $('#LocalFile', context).change(function (evt) {
        //alert("File changed");
        ...
        setBox('#IsFile');
        $('#uploadLogoAJAXButton').attr('disabled', true);
        $('#uploadLogoAJAXButton').css("visibility", "hidden");
        ...
        $('#IsUrl', context).change(function () {
            preview.attr('src', URLSource);
        });

        $('#IsFile', context).change(function () {
            var reader = new FileReader();
            reader.onload = function (e) { preview.attr('src', e.target.result); };
            reader.readAsDataURL(LocalFileSource);
        });
