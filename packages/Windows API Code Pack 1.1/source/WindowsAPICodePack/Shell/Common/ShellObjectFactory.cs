...
namespace Microsoft.WindowsAPICodePack.Shell
{
    internal static class ShellObjectFactory
    {
        ...
        internal static ShellObject Create(string parsingName)
        {
            ...
            IShellItem2 nativeShellItem = null;
            Guid guid = new Guid(ShellIIDGuid.IShellItem2);
            int retCode = -1;
            try
            {
                retCode = ShellNativeMethods.SHCreateItemFromParsingName(parsingName, IntPtr.Zero, ref guid, out nativeShellItem);
            }
            catch (Exception e)
            { }

            string path = parsingName;
            //IShellItem oItem;
            object oItem;
            ShellNativeMethods.SHCreateItemFromParsingName(path, IntPtr.Zero, typeof(IShellItem).GUID, out oItem);
